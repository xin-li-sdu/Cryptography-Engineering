...
void AES_Encrypt(uint8_t* plaintext, uint8_t* ciphertext, AES_Key Key) {
    ...
    for (int i = 1; i < Key.nr; i++) {
        ...
        tmp = TE[(s[2] >> 0) & 0xFF];
        t[3] ^= rotr32(tmp, 24);
        s[0] = t[0] ^ Key.ek[4 * i + 0];
        s[1] = t[1] ^ Key.ek[4 * i + 1];
        s[2] = t[2] ^ Key.ek[4 * i + 2];
        s[3] = t[3] ^ Key.ek[4 * i + 3];
    }
    ...
    s[0] = t[0] ^ Key.ek[4 * Key.nr + 0];
    s[1] = t[1] ^ Key.ek[4 * Key.nr + 1];
    s[2] = t[2] ^ Key.ek[4 * Key.nr + 2];
    s[3] = t[3] ^ Key.ek[4 * Key.nr + 3];
    for (int i = 0; i < 4; i++) {
...
void AES_Decrypt(uint8_t* ciphertext, uint8_t* plaintext, AES_Key Key) {
    ...
    s[0] ^= Key.dk[4 * Key.nr + 0];
    s[1] ^= Key.dk[4 * Key.nr + 1];
    s[2] ^= Key.dk[4 * Key.nr + 2];
    s[3] ^= Key.dk[4 * Key.nr + 3];
    for (int i = Key.nr - 1; i > 0; i--) {
        t[0] = TD[(s[0] >> 24) & 0xFF];
        tmp = TD[(s[3] >> 16) & 0xFF];
        t[0] ^= rotr32(tmp, 8);
        tmp = TD[(s[2] >> 8) & 0xFF];
        ...
        tmp = TD[(s[0] >> 0) & 0xFF];
        t[3] ^= rotr32(tmp, 24);
        s[0] = t[0] ^ Key.dk[4 * i + 0];
        s[1] = t[1] ^ Key.dk[4 * i + 1];
        s[2] = t[2] ^ Key.dk[4 * i + 2];
        s[3] = t[3] ^ Key.dk[4 * i + 3];
    }
